...
string ops = "-+*/^()";
string prior = "1122300";
string assoc = "llllrxx";
 ...
 *
 * @return 		Output RPN expression, space separated
 */
list<string> reorder(string str)
{
	int pos = 0;
	int prev = -1;
	...
	while(prev < pos) {
		...
		if(toke.lit != "") {
			...
			if(toke.op == '(') {
				...
				int opi1 = ops.find(toke.op);
				while(!opstack.empty()) {
					// Go ahead and evaluate higher priority operators before 
					// the current
					int opi2 = ops.find(opstack.front()); 
					if((assoc[opi1] == 'l' && prior[opi1] == prior[opi2]) || 
							prior[opi1] < prior[opi2]) {
						outqueue.push_back(makestring(opstack.front()));
						opstack.pop_front();
					} else {
						break;
